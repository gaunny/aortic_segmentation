...
def create_image(filename_img,direction):
    ...
    #images = images.reshape(im.shape[0],im.shape[1],im.shape[2],1)
    print(images.shape)
    #将切片数量放到第0维
    #images = images.reshape(-1, imgDim,imgDim,1)#images维度为[105,128,128,1]
    return images

# Load test data
#filelist_test = natural_sort(glob.glob('filter_coa_63_img_128_128.nii.gz')) # list of file names
filelist_test = natural_sort(glob.glob('data/Augment_data_resize/coa77/*_0_image.nii'))
print("====================== LOAD AXIAL NETWORK: ===========================")
t_start1 = time.time()
# Doing predictions with the model 
tf.reset_default_graph()      

new_saver = tf.train.import_meta_graph('Results/unet_axial_21_aug_105_filter_pad/model.ckpt.meta')

with tf.Session() as sess:
    new_saver.restore(sess, tf.train.latest_checkpoint('Results/unet_axial_21_aug_105_filter_pad/'))
    ...
    for i in range(len(filelist_test)):
        print('Processing test image', (i+1),'out of',len(filelist_test))
        ...
        np.savez('Results/Predictions/testprobmaps/coa77/test_prob_maps_axial_{}'.format(i),prob_maps=prob_maps)                            
