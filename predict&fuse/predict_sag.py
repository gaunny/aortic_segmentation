...
def create_image(filename_img,direction):
    ...
    #将切片数量放到第0维
    #images = images.reshape(-1, imgDim,imgDim,1)#images维度为[128,128,128,1]
...
print("====================== LOAD SAG NETWORK: ===========================")
t_start1 = time.time()
# Doing predictions with the model
tf.reset_default_graph()      

new_saver = tf.train.import_meta_graph('Results/unet_sag_21_aug_105_filter_pad/model.ckpt.meta')

with tf.Session() as sess:
    new_saver.restore(sess, tf.train.latest_checkpoint('Results/unet_sag_21_aug_105_filter_pad/'))
    ...
    for i in range(len(filelist_test)):
        ...
        prob_maps = []
        x_test = create_image(filelist_test[i],'sag')
        ...
        np.savez('Results/Predictions/testprobmaps/coa77/test_prob_maps_sag_{}'.format(i),prob_maps=prob_maps)                            
    print("================ DONE WITH TEST PREDICTIONS! ==================")  

t_end1 = time.time()
print("================ DONE WITH SAG PREDICTIONS! ==================")
